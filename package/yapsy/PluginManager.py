...
class PluginManager(object):
    ...
    def setPluginInfoExtension(self, ext):
        ...
        try:
            ...
            log.error(
                "Current plugin locator doesn't support setting the plugin info extension.")
            ...
            for element, element_name in ((getattr(candidate_module, name), name) for name in dir(candidate_module)):
                ...
                for category_name in self.categories_interfaces:
                    try:
                        ...
                        exc_info = sys.exc_info()
                        log.debug("correct subclass tests failed for: %s in %s" % (
                            element_name, candidate_filepath), exc_info=exc_info)
                        continue
